...
This model can generalize to unseen data better than the baseline model becaause it is less overfit. It is capable of factoring in various numerical and categorical features, and making a prediction based off boundaries in those features (similar to decision tree), instead of merely comparing to the most similar points in the dataset. Because of the lack of numerical features, its capacity for extrapolation only goes so far, since so many of the features are still binary Y/N answers.
